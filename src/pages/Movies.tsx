...
const Movies = () => {
  ...
  const fetchData = useCallback(async (pageNum: number) => {
    setIsLoading(true);
    try {
      const res = await animeService.getMovies(pageNum);
      if (res.data) {
        setResults(res.data);
      ...
      <div className="pt-24 max-w-7xl mx-auto px-6">
        <div className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div className="flex flex-col">
            <h2 className="text-white font-black uppercase text-2xl md:text-3xl tracking-tighter">Anime Movies Collection</h2>
            <div className="flex items-center gap-2 mt-1">
              <div className="w-2 h-2 rounded-full bg-[#EF4444] animate-pulse"></div>
              <span className="text-[10px] md:text-xs text-white/50 font-black uppercase tracking-widest">Kualitas Layar Lebar Terbaik</span>
            </div>
          </div>
