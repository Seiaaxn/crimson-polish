    ...
    >
      <div className="relative aspect-[3/4.5] w-full overflow-hidden bg-[#16161a] rounded-xl shadow-xl border border-white/5 group-hover:border-[#F6CF80]/30 transition-colors">
        {a?.isDevEntry && <div className="absolute top-2 right-2 bg-red-600/90 text-white text-[8px] font-black px-1.5 py-0.5 rounded shadow-lg z-20 tracking-wider border border-white/10">ND</div>}
          ...
          loading="lazy"
        />
        <div className="absolute top-2 left-2 bg-red-600 text-white px-2 py-0.5 rounded text-[8px] font-black uppercase z-20 flex items-center gap-1">
...
const Latest = () => {
  ...
  const fetchData = useCallback(async (pageNum: number) => {
    setIsLoading(true);
    try {
      const res = await animeService.getLatest(pageNum);
      if (res.data) {
        setResults(res.data);
      ...
      <div className="pt-24 max-w-7xl mx-auto px-6">
        <div className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div className="flex flex-col">
            <h2 className="text-white font-black uppercase text-2xl md:text-3xl tracking-tighter">Latest Update</h2>
            <div className="flex items-center gap-2 mt-1">
              <div className="w-2 h-2 rounded-full bg-red-600 animate-pulse"></div>
              <span className="text-[10px] md:text-xs text-white/50 font-black uppercase tracking-widest">Anime yang baru diupload</span>
            </div>
          </div>
